...
import { MAX_INPUT_COUNT, expandCatalog, validateTarget } from '../domain/catalog'

function createModel(catalog, request, glpk) {
  validateTarget(request.target)
  const entries = expandCatalog(catalog, undefined, request.excludeKids)
  const itemNames = [...new Set([...catalog.items, ...Object.keys(request.target)])].sort()
  ...
  const subjectTo = itemNames.map((itemName, itemIndex) => ({
    ...
    bnds: request.exact ? { type: glpk.GLP_FX, lb: request.target[itemName] || 0, ub: request.target[itemName] || 0 } : { type: glpk.GLP_LO, lb: request.target[itemName] || 0, ub: 0 },
  }))
